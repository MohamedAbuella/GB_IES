...
def complementarity_conditions(model):
    ...
    def technical_rule(model, i):
        price = model.pH if model.vector[i] == 'Hydrogen' else model.pE
        return pyompec.complements(
            model.q[i] >= 0,
            model.AF[i] * (price - model.c[i] - model.CO2 * model.emis[i]) - model.sigma[i] - model.alpha[i] <= 0)

    def investment_rule(model, i):
        expr = (-model.AF[i] * model.phi[i] * model.eps[i] - model.eps[i] *(1 - (model.lamda[i] - model.years) /
                (model.lamda[i] * (1 + model.rate[i])**model.years)) + model.sigma[i] - model.delta[i])
        return pyompec.complements(model.I[i] >= 0, expr <= 0)
    

    def technical_limit(model, i):
        return pyompec.complements(model.sigma[i] >= 0, model.q[i] - model.k[i] - model.I[i] <= 0)


    def investment_limit(model, i):
        return pyompec.complements(model.delta[i] >= 0, model.I[i] - model.iMAX[i] <= 0)


    def demand_balance_rule(model):
        return pyompec.complements(model.lambda_eq >= 0, sum(model.q[i] for i in model.p) - model.Q_e - model.Q_h >= 0)
    ...
    # model.c5 = pyompec.Complementarity(rule=demand_balance_rule)
...
def equality_constraints(model, systemData):
    
    if Players_option == 'ele_players':
        ...
        model.c6 = pyoen.Constraint(
            ...
            # expr=abs(sum(model.q[i] for i in selected_players) - model.Q_e) <= model.epsilon)
    ...
    elif Players_option == 'type_players':
        
        model.c6 = pyoen.Constraint(
            
        # expr = sum(model.q[i] for i in selected_players) == model.Q_e + model.Q_h)
        # expr = sum(model.q[i] for i in selected_players) <= model.Q_e + model.Q_h + 5)

        
        # expr = abs(abs(sum(model.q[i] for i in selected_players[:-2]) - model.Q_e)
        #             - abs(sum(model.q[i] for i in selected_players[-2:]) - model.Q_h)) <= 0.1)
        
        
        expr=abs(sum(model.q[i] for i in selected_players) - model.Q_e - model.Q_h) <= 0.1)


 
        # model.c7 = pyoen.Constrai1nt(
            
        # expr=abs(sum(model.q[i] for i in selected_players[:-2]) - model.Q_e) == 0)

        # model.c8= pyoen.Constraint(
            
        # expr=abs(sum(model.q[i] for i in selected_players[-2:]) - model.Q_h) == 0)
