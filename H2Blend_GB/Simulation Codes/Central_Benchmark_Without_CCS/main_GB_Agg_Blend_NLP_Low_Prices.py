...
from agg_blend_results_handler import *
from multinet_GB_Agg_Blend_Low_Pirces import *
...
def data_import(Scenarios):
    ...
    if CO2_penalty == 'FES':
        CO2_price = systemData['Carbon Price'].loc[systemData['Carbon Price']['Year'] == int(year), scenario].iloc[0]
    
    else: CO2_price = CO2_penalty
    
    systemData['CO2_penalty'] = CO2_price
    
    ###### Scaling for Hydrogen Blending, between 0 and 1 ######

    systemData['HB_NGB_share'] = HB_NGB_share
    
    EF_min = mdot_energy_conv(0)   # 14.62
    EF_max = mdot_energy_conv(1)   # 39.41
    
    EF=mdot_energy_conv(systemData['HB_NGB_share'])

    scaling_factor = (EF - EF_min) / (EF_max - EF_min)
    
    # systemData['scaling_factor'] = 1
    # systemData['scaling_factor'] = scaling_factor
    systemData['scaling_factor'] = HB_NGB_share
    ...
    systemData['Demand_h'] = systemData['Demand_h'] * systemData['scaling_factor']
    ...
    if G2G_sw=='Grey':
        systemData['Opex_Costs'] = pd.read_excel(GBdata_path + 'GB_Economic_Grey_H2.xlsx', sheet_name='Cost_Calc', index_col = 0)
    if G2G_sw=='Blue':
    ...
    if S4_Emiss==1 and gens_fg=="LwBESS_HiH2":
        gens_fg2="LwBESS_HiH2_emiss"
    
        systemData['Installed capacity'][2050] = systemData['Installed capacity'][gens_fg2] 
    
    if G2G_sw=='Grey':
        systemData['G2G_sw']='Grey'
    if G2G_sw=='Blue':
        systemData['G2G_sw']='Blue'
...
def parameter_list(model, year, systemData):
    ...
    systemData['CO2_penalty'] = CO2_price
    systemData['CfD_strike'] = cfd_value
    
    model.CO2 = pyoen.Param(initialize=(CO2_price))  #£/tonne CO2
    

    hydrogen_idx = [24, 25]  # p2g and g2g for hydrogen demand

    filtered_CfD = {
        i: cfd_value if (systemData['Agg_Players']['emissions'][i] == 0 or i in hydrogen_idx) else 0
        for i in model.p if i in systemData['Agg_Players']['emissions']}
    
    
    model.CfD = pyoen.Param(
        model.p,
        initialize=filtered_CfD,
        within=pyoen.NonNegativeReals,
        default=0)
    
    # model.CfD[6] = 55     # Offshore Wind
    # model.CfD[7] = 45     # Solar PV
    # model.CfD[4] = 92.5   # Nuclear
...
def calculate_shapley_values(model, num_samples=1000, debug_coalitions=None):
    ...
    def coalition_profits(coalition):
        ...
        for player in coalition:
            ...
            # CfD support: if strike > market, generator gets top-up
            cfd_strike = model.CfD[player]
            cfd_support = max(0, cfd_strike - price) * annual_output
            
            # revenue = annual_output * price
            revenue = annual_output * price + cfd_support
...
def distribute_generation_by_shapley(model, systemData):
    ...
    electric_players = [p for p in shapley_values if p not in [24, 25]]  # Exclude p2g (24) and g2g (25)
    hydrogen_players = [24, 25]  # Only p2g and g2g for hydrogen demand
...
def calculate_policy_opex_support(model, systemData, neg_shaps_list, demand_type='electric'):
    ...
    if demand_type=='electric':
        ...
        market_price = systemData['hydrogen_marginal_cost']
...
def calculate_policy_npv_support(model, systemData, neg_shaps_list, demand_type='electric'):
    ...
    for p in allocated_group:
        ...
        cfd_strike = model.CfD[p]
        cfd_support = max(0, cfd_strike - market_price) * q
                        
        # revenue = q * market_price
        revenue = q * market_price + cfd_support
...
def calculate_policy_cfd_support(model, systemData, demand_type='electric'):
    
    global policy_support_cfd, dic_supports_cfd_e, dic_supports_cfd_h
    
    # Separate players by type
    electric_players = [p for p in shapley_values if p not in [24, 25]]  # Exclude p2g (24) and g2g (25)
    hydrogen_players = [24, 25]  # Only p2g and g2g for hydrogen demand
    cfd_players = [i for i, v in model.CfD.extract_values().items() if v > 0]

    # Separate by demand type, only include players with CfD > 0
    if demand_type == 'electric':
        allocated_group = [p for p in electric_players if p in cfd_players]
        market_price = model.pE.value
    elif demand_type == 'hydrogen':
        allocated_group = [p for p in hydrogen_players if p in cfd_players]
        # market_price = model.pH.value
        market_price = systemData['hydrogen_marginal_cost']
        

    dic_supports = {}
    policy_support_cfd = 0.0
    ...
    for p in allocated_group:
        ...
        cfd_strike = model.CfD[p]
        # cfd_support = max(0, cfd_strike - market_price) * 
        cfd_support = (cfd_strike - market_price) * q
        ...
        policy_support_cfd += cfd_support
        dic_supports[p] = cfd_support
        
        
    systemData['CfD_support'] = {
        # p: max(0, model.CfD[p] - market_price) * model.q[p].value
        p:  (model.CfD[p] - market_price) * model.q[p].value
        for p in allocated_group}
    
    dic_support_cfd2 = systemData['CfD_support']
    

    if demand_type == 'electric':
        dic_supports_cfd_e = dic_supports
    elif demand_type == 'hydrogen':
        dic_supports_cfd_h = dic_supports
    

    return policy_support_cfd
...
def system_profit_objective(model):
    return sum(
        (
            model.q[i] * (
                model.pH if model.vector[i] == 'Hydrogen' else model.pE
            )
            - model.c[i] * model.q[i]
            - model.CO2 * model.emis[i] * model.q[i]
        ) / model.AF[i]
        - model.phi[i] * model.eps[i] * model.I[i]
        for i in model.p
    )
...
def cooperative_game(year, scenario, data):
    ...
    model.SystemProfit = Objective(rule=system_profit_objective, sense=maximize)
...
def initial_run_OPGF(model, systemData):
    ...
    for tech in systemData['G2P Data']:
        systemData['G2P Data'][tech]['Capacity(GW)'] *= systemData['scaling_factor']
...
def cost_check(model, multinet, systemData):
    ...
    # G2G_out = systemData['Agg_Players'].loc[systemData['Agg_Players']['type'] == 'G2G+CCS', 'max_p_mw'].iloc[0]
    G2G_out = systemData['Agg_Players'].loc[systemData['Agg_Players']['type'] == 'g2g', 'max_p_mw'].iloc[0]

    if G2G_sw=='Grey': 
        G2G_emiss = G2G_out * 0.32
    if G2G_sw=='Blue': 
        G2G_emiss = G2G_out * 0
        
    emission_OPGF = sum(multinet['nets']['power']['res_gen']['p_mw'] * players['emissions'])
    
    emission_OPGF = emission_OPGF + G2G_emiss
    ...
    p2g_out = multinet.nets['hydrogen'].res_source['mdot_kg_per_s'][:51].sum(skipna=True) * (39.41 * 3600) / 1000

    g2p_in = multinet.nets['hydrogen'].res_sink['mdot_kg_per_s'][14:].sum(skipna=True) * (39.41 * 3600) / 1000
    ...
    Total_Cost_G2G = systemData['C_ATR'] * g2g_out
    Total_Cost_P2G = systemData['C_EZ'] * p2g_out
    Total_Cost_G2P = systemData['hydrogen_marginal_cost'] * g2p_in
    

    cost_GT = cost_GT + co2_cost_GT + g2gCost
    
    ## cost_OPGF = cost_OPGF + co2_cost_OPGF + g2gCost 
    cost_OPGF = cost_OPGF + co2_cost_OPGF + Total_Cost_G2G + Total_Cost_P2G + Total_Cost_G2P 

    # print('C_EZ', systemData['C_EZ'])
    # print('C_ATR', systemData['C_ATR'])
    # print('hydrogen_marginal_cost', systemData['hydrogen_marginal_cost'])

    # print('Total_Cost_G2G', Total_Cost_G2G)
    # print('Total_Cost_P2G', Total_Cost_P2G)
    # print('Total_Cost_G2P', Total_Cost_G2P)
    
    # print('cost_OPGF_1', cost_OPGF + co2_cost_OPGF + g2gCost)
    ...
    return {
        ...
        "marg_cost_H2": systemData['hydrogen_marginal_cost'],
        "subs_cost_H2": systemData['hydrogen_subsidised_cost'],
...
def update_GT(year, scenario, multinet, systemData):
    ...
    from agg_blend_results_handler import OPGF_results
...
def get_selected_players(selection_option):
    ...
    # print('sty_Agg_Players', systemData['Agg_Players'])
    # st=stop
...
#################################################
    # Set globals
    global phi_fg, gens_fg, invest_span, price_list, CO2_penalty, cfd_value, S4_Emiss, G2G_sw, HB_NGB_share

    # HB_NGB_share = 0.0  ## Proportion of H2 into NG network by volume (m3 or mole fraction, % vol H₂ in the mixture)
    # HB_NGB_share = 0.01
    # HB_NGB_share = 0.1
    # HB_NGB_share = 0.2
    HB_NGB_share = 1.0
    ...
    # Figs_Saving_sw = 'Figs_Saving'
    Figs_Saving_sw = 'No_Figs_Saving'
    
    # C_EZ_cfd_sw = 'Sub_H2green' # Subsidised green hydrogen 
    C_EZ_cfd_sw = 'No_SubH2green'


    # G2G_sw='Blue' # Type of hydrogen from gas reforming (Blue or Grey)
    G2G_sw='Grey'
    
    
    ### if C_EZ_cfd_sw =='No_SubH2green':
        
    # CO2_penalties = [0, 100, 200, 300, 400, 500]    
    # cfd_values = [0, 10, 25, 50, 100, 200, 250]
    
    
    # ### if C_EZ_cfd_sw =='SubH2green':
    # CO2_penalties = [0, 100, 250, 400]

    # cfd_values = [0, 10, 20, 30, 40]
    ...
    price_levels = {
    # "high": [100, 75, 150],   # Elec, Gas, Hydrogen
    # "low":  [40, 15, 50]
    # "low":  [40, 15, 70]
    "low":  [50, 37, 100]
    }    
    ...
    for price_level, price_list in price_levels.items():
        print(f"\n=== Starting runs for price level: {price_level.upper()} ===")
    
        from itertools import product
        for CO2_penalty, cfd_value in product(CO2_penalties, cfd_values):
            print(f"Running model ({price_level}) with CO2_penalty={CO2_penalty}, cfd_value={cfd_value}")
            ...
            S4_Emiss=0 #Senario4: LwBESS_HiH2 without CO2 emissions
            # S4_Emiss=1 #Senario4: LwBESS_HiH2 with CO2 emissions
        ...
        #################################################    
            ...
            for gens_flag, scenario_name in plan_scenarios.items():
                ...
                systemData['C_EZ_cfd_sw'] = C_EZ_cfd_sw
                systemData['Figs_Save_sw'] = Figs_Saving_sw
                ...
                from agg_blend_results_handler import update_gt_with_opgf_results
                ...
                Case_name = 'CfD_Central'  
                
                
                if G2G_sw=='Grey':
                    Case_name = 'CfD_GreyH2_Central'  
                    if C_EZ_cfd_sw =='Sub_H2green':
                        Case_name = 'CfD_GreyH2_Sub_H2green_Central'  
                    if C_EZ_cfd_sw =='No_SubH2green':
                        Case_name = 'CfD_GreyH2_Central'  
                        
                if G2G_sw=='Blue':
                    Case_name = 'CfD_BlueH2_Central'  
                    if C_EZ_cfd_sw =='Sub_H2green':
                        Case_name = 'CfD_BlueH2_Sub_H2green_Central'  
                    if C_EZ_cfd_sw =='No_SubH2green':
                        Case_name = 'CfD_BlueH2_Central' 
        
                if S4_Emiss==1:
                    Case_name = 'CfD_S4Emiss_Central'
                    
                if phi_fg==0.0:phi_nm = 'Variable'
                else: phi_nm = phi_fg
                                
                
                if CO2_penalty=='FES': CO2_nm = round(systemData['Carbon Price'].loc[systemData['Carbon Price']['Year'] == int(year), scenario].iloc[0])
                else: CO2_nm = CO2_penalty
                ...
                # Subfolders = {'': Case_name} | {'phi': f"{phi_nm}"} | {'PI': f"CfD_{cfd_value}_CO2_{CO2_nm}"} | {'price': f"{price_level}"} | {'Case': f"{gens_fg}"} | Case_Demand
                Subfolders = {'': Case_name} | {'H2_prop': f"{HB_NGB_share}"} | {'PI': f"CfD_{cfd_value}_CO2_{CO2_nm}"} | {'price': f"{price_level}"} | {'Case': f"{gens_fg}"} | Case_Demand
                ...
                from agg_blend_results_handler import process_policy_support_and_save
                ...
                process_policy_support_and_save(model, multinet, systemData, df_shap, output_folder,
                                             ...
                                             calculate_policy_opex_support, calculate_policy_npv_support, calculate_policy_cfd_support)
            
                from agg_blend_results_handler import run_scenario_simulation
        
                run_scenario_simulation(phi_fg, systemData, invest_span, gens_flag, time_steps, Scenarios['Demand_level'], output_folder)
                
                # st=Stop
                
                print(f"Finished ({price_level}): CO2_penalty={CO2_penalty}, cfd_value={cfd_value}")
