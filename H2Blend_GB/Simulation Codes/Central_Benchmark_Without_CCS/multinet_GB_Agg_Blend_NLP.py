...
def data_import(Scenarios):
    ...
    if G2G_sw=='Grey':
        Economics_data = pd.read_excel(GBdata_path + 'GB_Economic_Grey_H2.xlsx', sheet_name='Econ_Players', index_col = 0)
    if G2G_sw=='Blue':
    ...
    if G2G_sw == 'Grey':
        systemData['Economics_H2'] = pd.read_excel(GBdata_path + 'GB_Economic_Grey_H2.xlsx', sheet_name='Econ_H2', index_col=0)

    elif G2G_sw == 'Blue':
        systemData['Economics_H2'] = pd.read_excel(GBdata_path + 'GB_Economic_Parameters.xlsx', sheet_name='Econ_H2', index_col=0)
    ...
    if G2G_sw=='Grey':
        systemData['Players_H2'] = pd.read_excel(GBdata_path + 'GB_Economic_Grey_H2.xlsx', sheet_name='Econ_Players_H2', index_col = 0)
    if G2G_sw=='Blue':
...
def power_network(time_steps, systemData):
    ...
    CO2_penalty = systemData['CO2_penalty']

    systemData['Gen Cost']['5'] = systemData['Players']['costs'] + (systemData['Players']['emissions'] * CO2_penalty)


    idx_stor = range(13, len(systemData['Gen Data']['8']), 21)
    systemData['Gen Data']['8'][idx_stor] = 2.5 * systemData['Gen Data']['8'][idx_stor]
...
def hydrogen_network(time_steps, systemData):
    ...
    print('systemData_HB_NGB_share', systemData['HB_NGB_share'])

    HB_NGB_share = 1-systemData['HB_NGB_share']
    ...
    mixture_properties = calculate_mixture_properties('lgas', 'hydrogen', HB_NGB_share, temp_system)
    ...
    ##### Scaling for Hydrgoen Blending ##### 
    # sc_f = 1
    sc_f = systemData['scaling_factor']

    demand_data = [x * sc_f for x in demand_data]
    
    # Scale Pandas Series (convert to numeric first, keep Series form)
    demand_h_all = pd.to_numeric(demand_h_all, errors='coerce') * sc_f

    # print('load_h', load_h)
    # print('demand_h', demand_h)
    # print('demand_data', demand_data)
    # print('demand_h_all', demand_h_all)
    # print('scaling factor', systemData['scaling_factor'])
    ...
    for from_acronym, to_acronym, capacity in connections:
        ...
        ppipes.create_pipe_from_parameters(
            net, from_junction=junctions[from_acronym], to_junction=junctions[to_acronym],
            length_km=distance_km, k_mm=0.025, diameter_m=1, name=f"{from_acronym}-{to_acronym}"
        )
...
def coupling_g2p(net_power, net_hyd, multinet, systemData):
    ...
    total_P2G_capacity = sum(df['Capacity(GW)'].sum() for df in systemData['P2G Data'].values())
    total_G2G_capacity = sum(df['Capacity(GW)'].sum() for df in systemData['G2G Data'].values())
    total_G2P_capacity = sum(df['Capacity(GW)'].sum() for df in systemData['G2P Data'].values())

    if systemData['run_multinet'] ==0:
        print('total_G2G_capacity_0', total_G2G_capacity)   
    if systemData['run_multinet'] ==1:
        print('total_P2G_capacity_1', total_P2G_capacity)
        print('total_G2G_capacity_1', total_G2G_capacity)
        print('total_G2P_capacity_1', total_G2P_capacity)

        print('Sum_total_P2G_G2G_1', total_P2G_capacity + total_G2G_capacity)

    if systemData['run_multinet'] ==2:
        total_P2G_capacity = sum(df['Capacity(GW)'].sum() for df in systemData['P2G Data'].values())
        total_G2G_capacity = sum(df['Capacity(GW)'].sum() for df in systemData['G2G Data'].values())
        total_G2P_capacity = sum(df['Capacity(GW)'].sum() for df in systemData['G2P Data'].values())
        print('total_P2G_capacity_2', total_P2G_capacity)
        print('total_G2G_capacity_2', total_G2G_capacity)
        print('total_G2P_capacity_2', total_G2P_capacity)

        print('Sum_total_P2G_G2G_2', total_P2G_capacity + total_G2G_capacity)
...
def coupling_p2g_2(net_power, net_hyd, multinet, systemData):
    p2g_data = systemData['P2G Data']

    # Region → power bus index mapping
    region_to_bus = {
        region: int(bus)
        for region, bus in zip(regions.keys(), net_power['bus']['name'] - 1)
    }

    for tech_name, df in p2g_data.items():
        for _, row in df.iterrows():
            region = row["Region"]
            cap_gw = row["Capacity(GW)"]      # GW installed
            efficiency = float(row["Efficiency"])

            if cap_gw <= 0 or region not in region_to_bus:
                continue

            # Hydrogen output power in MW
            h2_output_mw = cap_gw * 1000 * efficiency

            # Convert MW → kg/s using your method
            mdot_h2 = convert_mwh_to_mdot(h2_output_mw, energy_hydrogen)

            # Electrical input required (MW)
            elec_input_mw = h2_output_mw / efficiency

            # Add fixed electricity load
            ppower.create_load(
                net_power,
                bus=region_to_bus[region],
                p_mw=elec_input_mw,
                name=f"P2G_load_{region}_{tech_name}"
            )

            # Add hydrogen source
            hyd_junc = net_hyd['junction'].loc[
                net_hyd['junction']['name'] == region
            ].index[0]

            ppipes.create_source(
                net_hyd,
                junction=hyd_junc,
                mdot_kg_per_s=mdot_h2,
                name=f"P2G_H2_source_{region}_{tech_name}"
            )
...
def coupling_g2g_2(net_power, net_gas, net_hyd, multinet, systemData):
    ...
    region_to_gas_junc = {
        row['name']: idx
        for idx, row in net_gas['junction'].iterrows()
    }

    for tech_name, df in g2g_data.items():
        for _, row in df.iterrows():

            region = row["Region"]
            cap_gw = row["Capacity(GW)"]
            efficiency = float(row["Efficiency"])

            if cap_gw <= 0:
                continue
            if region not in region_to_bus or region not in region_to_gas_junc:
                continue

            # Hydrogen output (MW)
            h2_output_mw = cap_gw * 1000 * efficiency

            # Convert to H₂ mass flow rate
            mdot_h2 = convert_mwh_to_mdot(h2_output_mw, energy_hydrogen)

            # Required NG thermal input (MW)
            ng_input_mw = h2_output_mw / efficiency

            # --- Electricity load from CCS ---
            if tech_name == "ATRCCS":
                ccs_load_mw = ng_input_mw * 0.2
            elif tech_name == "BECCS":
                ccs_load_mw = ng_input_mw * 0.3
            else:
                ccs_load_mw = 0
            ...
            ppower.create_load(
                ...
                bus=region_to_bus[region],
                p_mw=ccs_load_mw,
                name=f"{tech_name}_{region}_CCS_load"
            )

            # --- Natural Gas sink ---
            mdot_ng = convert_mwh_to_mdot(ng_input_mw, energy_gas)

            ppipes.create_sink(
                net_gas,
                junction=region_to_gas_junc[region],
                mdot_kg_per_s=mdot_ng,
                name=f"{tech_name}_{region}_NG_sink"
            )
            ...
            ppipes.create_source(
                ...
                junction=hyd_junc,
                mdot_kg_per_s=mdot_h2,
                name=f"{tech_name}_{region}_H2_source"
            )
...
def split_ez_atr(systemData):
    
    CO2_penalty = systemData['CO2_penalty']
    cfd_value = systemData['CfD_strike'] 
    model = systemData['model'] 
    G2G_sw = systemData['G2G_sw']
    C_EZ_cfd_sw=systemData['C_EZ_cfd_sw']
    
    p2g_cap = model.iMAX.extract_values()[24]
    g2g_cap = model.iMAX.extract_values()[25]
    
    ele_opex_price =  model.pE.value # £/MWh
    gas_opex_price =  model.pG.value # £/MWh
    ...
    #### Using algebraic ratio, for long-term, opex-based hydrogen split (annual average).  Since our model uses average energy prices and efficiencies, the algebraic ratio is fully justified.
    
    C_EZ_cfd = (ele_opex_price / ez_eff) - cfd_value # Subsidising cost green  hydrogen 

    C_EZ = ele_opex_price / ez_eff 

    if G2G_sw=='Blue':
        C_ATR = gas_opex_price / atr_eff + CO2_penalty * 0

    if G2G_sw=='Grey':
        C_ATR = gas_opex_price / atr_eff + CO2_penalty * 0.32  

    
    C_EZ_cfd = max(C_EZ_cfd, 1e-6)
    C_ATR = max(C_ATR, 1e-6)
    
    
    if C_EZ_cfd_sw=='Sub_H2green':
    
        f_EZ  = 1 / C_EZ_cfd / (1 / C_EZ_cfd + 1 / C_ATR)
        f_ATR = 1 - f_EZ
    
    if C_EZ_cfd_sw=='No_SubH2green':
    ...
    hydrogen_subsidised_cost = f_EZ * C_EZ_cfd + f_ATR * C_ATR  #Cost of Subsidised Hydrogen cost
    systemData['hydrogen_subsidised_cost'] = hydrogen_subsidised_cost
    ...
    print('CO2_penalty', CO2_penalty)    
    print('model.pE', model.pE.value)
    print('model.pG', model.pG.value)
    print('Hyd_marg_cost_hydrogen', systemData['hydrogen_marginal_cost'])
    print("EZ/ATR split: EZ_share=", f_EZ, " ATR_share=", f_ATR)
    
    #@CO2_penalty=500 EZ/ATR split: EZ_share= 0.61  ATR_share= 0.39, Hyd_marg_cost = 203.125

    ###Calculate the hydrogen capacities for the electrolysers and  reformers 
    total_P2G_capacity = sum(df['Capacity(GW)'].sum() for df in systemData['P2G Data'].values())
    total_G2G_capacity = sum(df['Capacity(GW)'].sum() for df in systemData['G2G Data'].values())

    
    # scaling_factor = 1
    scaling_factor = systemData['scaling_factor'] 
    
    ##### Apply scaling #####
    P2G_scale_factor = f_EZ
    G2G_scale_factor = f_ATR
    total_system_capacity = total_P2G_capacity + total_G2G_capacity
    P2G_scale_factor = f_EZ * total_system_capacity / total_P2G_capacity * scaling_factor
    G2G_scale_factor = f_ATR * total_system_capacity / total_G2G_capacity * scaling_factor
    
        
    # Scale P2G capacities 
    for tech, df in systemData['P2G Data'].items(): 
        df['Capacity(GW)'] = df['Capacity(GW)'] * P2G_scale_factor 

    # Scale G2G capacities 
    for tech, df in systemData['G2G Data'].items(): 
        df['Capacity(GW)'] = df['Capacity(GW)'] * G2G_scale_factor
    ###################  
        
    H2_subsidy_per_MWh = cfd_value - hydrogen_marginal_cost
    Green_H2_subsidy_per_MWh = min(cfd_value, C_EZ)
    Total_H2_subsidy = H2_subsidy_per_MWh * (total_P2G_capacity + total_G2G_capacity)*1000 # in m£    
    Total_Green_H2_subsidy = Green_H2_subsidy_per_MWh * (total_P2G_capacity)*1000 # in m£

    systemData['H2_subsidy_per_MWh'] = H2_subsidy_per_MWh
    systemData['Green_H2_subsidy_per_MWh'] = Green_H2_subsidy_per_MWh
    systemData['Total_H2_subsidy'] = Total_H2_subsidy
    systemData['Total_Green_H2_subsidy'] = Total_Green_H2_subsidy
    
    
    # ==============================
    # ---- TOTAL Capacity COST CALCULATION ---
    # ==============================
    
    P2G_energy_MWh = total_P2G_capacity * 1000
    G2G_energy_MWh = total_G2G_capacity * 1000
    
    Total_Cost_P2G_max = C_EZ * P2G_energy_MWh
    Total_Cost_G2G_max = C_ATR * G2G_energy_MWh
    Total_Cost_G2P_max = hydrogen_marginal_cost * (P2G_energy_MWh + G2G_energy_MWh)
    
    # Save into dictionary
    systemData['C_EZ'] = C_EZ
    systemData['C_ATR'] = C_ATR
    systemData['G2G_energy_MWh'] = G2G_energy_MWh
    systemData['P2G_energy_MWh'] = P2G_energy_MWh

    systemData['Total_Cost_P2G_max'] = Total_Cost_P2G_max
    systemData['Total_Cost_G2G_max'] = Total_Cost_G2G_max
    systemData['Total_Cost_G2P_max'] = Total_Cost_G2P_max
    
    
    print("Total_Cost_P2G_max", Total_Cost_P2G_max)
    print("Total_Cost_G2G_max", Total_Cost_G2G_max)
    print("Total_Cost_G2P_max", Total_Cost_G2P_max)
    
    print('hydrogen_marginal_cost', hydrogen_marginal_cost)
    print('H2_subsidy_per_MWh', H2_subsidy_per_MWh)
    print('Green_H2_subsidy_per_MWh', Green_H2_subsidy_per_MWh)
    print('Total_H2_subsidy', Total_H2_subsidy)
    print('Total_Green_H2_subsidy', Total_Green_H2_subsidy)
    

    print('HB_Share', systemData['HB_NGB_share'])
    
    print('total_P2G_capacity', total_P2G_capacity)
    print('total_G2G_capacity', total_G2G_capacity)
    
    print('P2G_scale_factor', P2G_scale_factor)
    print('G2G_scale_factor', G2G_scale_factor)
...
def run_OPGF(t, genData,  systemData):
    ...
    for ti in [t]:
        ...
        global multinet, run_multinet
        
        systemData['run_multinet'] = 1
        
        split_ez_atr(systemData)
...
def form_multinet(net_power, net_gas, net_hyd, systemData):
    ...
    coupling_p2g_2(net_power, net_hyd, multinet, systemData)
    coupling_g2g_2(net_power, net_gas, net_hyd, multinet, systemData)
